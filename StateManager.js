...
class StateManager {
    ...
    dispatch(action, middlewares) {
        if(!action.type || !isObject(action)) throw Error('Action must be an object with a type property');
        if(this.middlewares.length > 0) {
            this.middlewares.forEach(middleware => {
                isFunction(middleware) ? middleware(this.state) : null 
            })
        }
